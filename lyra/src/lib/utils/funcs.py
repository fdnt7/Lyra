...
class LyraConfig(t.NamedTuple):
    ...
    is_dev_mode: bool
    token: str  # TODO: Change this to `typing.LiteralString` once 3.11 is out
    decl_glob_cmds: bool | SequenceOr[int]
    emoji_guild: int
...
async def say(
    ...
    **kwargs: t.Any,
    ...
    try:
        ...
        flags = hk.MessageFlag.EPHEMERAL if hidden else hk.UNDEFINED
...
def trigger_thinking(
    ...
    *,
    ephemeral: bool = False,
    flags: hk.UndefinedOr[int | hk.MessageFlag] = hk.UNDEFINED,
...
async def restricts_c(
    ...
    if author_perms & (hk.Permissions.ADMINISTRATOR | RESTRICTOR):
        return True
...
async def on_error(ctx: tj.abc.Context, error: Exception) -> bool:
    from ..errors import BaseLyraError

    if isinstance(error, BaseLyraError):
        pass
...
async def pre_execution(
    ...
    if bot_perms & hk.Permissions.MANAGE_MESSAGES:
